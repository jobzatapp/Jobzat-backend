const { User, Candidate, Employer, CandidateProfile } = require('../models');
...
const register = async (req, res) => {
    try {
        ...
        return res.status(201).json({
            ...
            token,
            user: { id: user.id, email: user.email, role: user.role, role_added: user.role_added, profile_completed: user.profile_completed, is_verified: user.is_verified }
        });
    } catch (err) {
...
const getMe = async (req, res) => {
    try {
        const user = await User.findByPk(req.user.id, {
            include: [
                { model: Candidate, as: 'candidate', include: [{ model: CandidateProfile, as: 'profile' }] },
                { model: Employer, as: 'employer' },
